...
#获得oracle数据库连接
def get_conn(ora_info='aueic/ftuser@XIN_KAIFA'):
    try:
...
def p_judge_run(inc = 30): 
    try:
        ...
        cursor = db_conn.cursor () 
        str_out = cursor.arrayvar(cx_Oracle.STRING,[None]*10)
        ...
        cursor.callproc('aueic.pkg_etl_model.p_judge_run',[str_out,str2,str3,str4,str5])
